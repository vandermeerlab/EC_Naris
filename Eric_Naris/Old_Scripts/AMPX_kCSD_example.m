...
%% test inversion
if cfg.test
    disp('USING TEST INVERSION')
    filt_data(37,:) = -1*(filt_data(37,:));
filt_data(46,:) = -1*(filt_data(46,:));
filt_data(55,:) = -1*(filt_data(55,:));    
filt_data(64,:) = -1*(filt_data(64,:));
    figure; hold on
    for iChan = 1:8
        plot(filt_data(cfg.chan_to_plot(iChan),:), 'color', c_ord(iChan,:))
    end
...
samples = 1:nSamples;
tic
for iS = 450:800% 1:length(samples);
